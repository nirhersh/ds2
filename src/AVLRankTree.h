...
public:
    ...
    */
    ...
    AVLRankTree(const AVLRankTree<T, K>& other) = delete;
    
    AVLRankTree<T, K>& operator=(const AVLRankTree<T, K>& other) = delete;
    ...
    */
    ...
    static void unite_trees(AVLRankTree<T, K>& tree1, AVLRankTree<T, K>& tree2, AVLRankTree<T, K>& newTree);
...
private:
    ...
    void update_rank(Node* leaf, bool toIncrease);
    
};

template <class T, class K>
class AVLRankTree<T, K>::Node{
public:
...
private:
    ...
    friend class AVLRankTree<T, K>;
};

template <class T, class K>
bool AVLRankTree<T, K>::is_empty()const{
...
//-------------------- Node Implemantation --------------------
template <class T, class K>
AVLRankTree<T, K>::Node::Node(T* data, const K& key) : m_data(data), m_key(key), m_height(0),
                         m_right(nullptr), m_left(nullptr), m_parent(nullptr), m_rank(0){}

template <class T, class K>
K AVLRankTree<T, K>::Node::get_key()
{
    return m_key;
}

//-------------------- AVLRankTree Implemantation --------------------
template <class T, class K>
AVLRankTree<T, K>::AVLRankTree(): root(nullptr), m_size(0){}

template <class T, class K>
T* AVLRankTree<T, K>::search(const K& key) const{
    Node* nodeToReturn = search_node(key);
    return nodeToReturn->m_data;
}

template <class T, class K>
void AVLRankTree<T, K>::unite_trees(AVLRankTree<T, K>& tree1, AVLRankTree<T, K>& tree2, AVLRankTree<T, K>& newTree)
{

    int tree1Size = tree1.get_size();
    int tree2Size = tree2.get_size();
    typename AVLRankTree<T, K>::Node** arrayOfTree1 = nullptr;
    typename AVLRankTree<T, K>::Node** arrayOfTree2 = nullptr;
    if(tree1Size == 0 && tree2Size == 0){
        return;
    }
    if(tree1Size != 0){
        arrayOfTree1 = new typename AVLRankTree<T, K>::Node*[tree1Size];
        for(int i=0; i<tree1Size; i++){
            arrayOfTree1[i] = nullptr;
        }
    } 
    if(tree2Size != 0){
        arrayOfTree2 = new typename AVLRankTree<T, K>::Node*[tree2Size];
    ...
    typename AVLRankTree<T, K>::Node** newArrayTree = new typename AVLRankTree<T, K>::Node*[tree1Size + tree2Size];
...
template <class T, class K>
void AVLRankTree<T, K>::merge(typename AVLRankTree<T, K>::Node* arrayTree1[], int na, typename AVLRankTree<T, K>::Node* arrayTree2[], 
                                                      int nb, typename AVLRankTree<T, K>::Node* newArrayTree[])
{
    int ia, ib, ic;
    try{
...
template <class T, class K>
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::array_to_tree(typename AVLRankTree<T, K>::Node** arrayTree, int start, int end)
{
    if(start > end){
        return nullptr;
    }
    ...
    typename AVLRankTree<T, K>::Node* newNode (new typename AVLRankTree<T, K>::Node(arrayTree[mid]->m_data, arrayTree[mid]->m_key ));
...
template <class T, class K>
void AVLRankTree<T, K>::fix_tree_height(Node* node)
{
    if(!node){
        return;
    }
...
template <class T, class K>
bool AVLRankTree<T, K>::check_balance_factor(Node* node)
{
    if(!node)
    {
        return true;
...
template <class T, class K>
void AVLRankTree<T, K>::push(T* item, const K& key){
    assert(item != nullptr);
    bool found = false;
...
template<class T, class K>
void AVLRankTree<T, K>::balance_tree(Node* newNode)
{
...
template<class T, class K>
void AVLRankTree<T, K>::LL_fix(Node* parent){
    Node* B = parent;
...
template<class T, class K>
void AVLRankTree<T, K>::LR_fix(Node* parent)
{
...
template<class T, class K>
void AVLRankTree<T, K>::RL_fix(Node* parent){
    LL_fix(parent->m_right);
    RR_fix(parent);
}

template<class T, class K>
void AVLRankTree<T, K>::RR_fix(Node* parent)
{
    Node* B = parent;
...
template<class T, class K>
int AVLRankTree<T, K>::calc_BF(Node* node)
{
    if(node == nullptr){
        return -2;
    }
...
template<class T, class K>
void AVLRankTree<T, K>::fix_height(Node* node1)
{
    if(!node1){
        return;
    }
...
template<class T, class K>
void AVLRankTree<T, K>::remove(const K& key)
{
...
template<class T, class K>
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::search_node(const K& key) const
{
    bool found = false;
    Node* currentNode = root;
    while (!found)
...
template<class T, class K>
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::remove_node(Node* nodeToRemove){
    bool isRight = false;
    assert(nodeToRemove);
    ...
    if (nodeToRemove->m_right == nullptr && nodeToRemove->m_left == nullptr)
    {
        ...
        update_rank(nodeToReturn, false);
        delete nodeToRemove; 
    }
        ...
        update_rank(nodeToReturn->m_parent, false);
        delete nodeToRemove;
    }
    else if(nodeToRemove->m_right == nullptr && nodeToRemove->m_left != nullptr){
        if(nodeToRemove != root){
...
template<class T, class K>
void AVLRankTree<T, K>::switch_nodes(Node* node1, Node* node2){
...
template<class T, class K> 
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::get_following_node(Node* node) const //if there is no right son return the parent
{
    if(node == nullptr){
        throw KeyDoesntExists();
    }
...
template<class T, class K>
int AVLRankTree<T, K>::get_size()const{
    return m_size;
}

template<class T, class K>
void AVLRankTree<T ,K>::in_order(T** array) const{
    if(array == nullptr){
        return;
    }
...
template<class T, class K>
void AVLRankTree<T ,K>::in_order(Node* array[]) const{
    if(array == nullptr){
        return;
    }
    in_order_recursion(array, root, 0);
...
template<class T, class K>
int AVLRankTree<T, K>::in_order_recursion(T** array, Node* current, int index) const
{
    if(current == nullptr){
        return index;
    }
...
template<class T, class K>
int AVLRankTree<T, K>::in_order_recursion(Node* array[], Node* current, int index) const
{
    if(current == nullptr){
        return index;
    }
...
template<class T, class K>
bool AVLRankTree<T, K>::exists(const K& key) const
{
    try{
        search_node(key);
...
template<class T, class K>
T* AVLRankTree<T, K>::get_following_value(const K& key) const{
    if(is_empty()){
        throw EmptyTree();
    }
...
template<class T, class K>
T* AVLRankTree<T, K>::get_preceding_value(const K& key) const{
    if(is_empty()){
        throw EmptyTree();
    }
...
template<class T, class K> 
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::get_preceding_node(Node* node) const
{
    if(node == nullptr){
        throw KeyDoesntExists();
    }
...
template<class T, class K> 
void AVLRankTree<T, K>::recurtion_dtor(Node* node)
{
    if(!node){
        return;
    }
...
template<class T, class K> 
int AVLRankTree<T, K>::get_height() const
{
    fix_tree_height(root);
...
template<class T, class K> 
typename AVLRankTree<T, K>::Node* AVLRankTree<T, K>::getRoot() const
{
    return root;
}

template<class T, class K> 
T* AVLRankTree<T, K>::binary_search_closest(K key)
{
    if(is_empty()){
        return nullptr;
    }
...
template<class T, class K> 
void AVLRankTree<T, K>::update_rank(Node* leaf, bool toIncrease)
{
...
T* AVLRankTree<T, K>::get_item_by_index(int index)
{
    ...
    {
        assert(currentNode);
        if(currentNode->m_rank == index){
            while (currentNode->m_right)
            {
                currentNode = currentNode->m_right;
            }
            return currentNode->get_data();
        }
        if((currentNode->m_left && currentNode->m_left->m_rank == index - 1) || (index == 1 && !(currentNode->m_left))){
            return currentNode->get_data();
        }
        if(currentNode->m_left && currentNode->m_left->m_rank >= index){
            currentNode = currentNode->m_left;
        }else {
            index -= currentNode->m_left? currentNode->m_left->m_rank + 1 : 1;
            currentNode = currentNode->m_right;
            assert(index >= 0);
        }
